...
export const ShoppingView: React.FC<ShoppingViewProps> = ({ lists, onSaveLists, showToast, onShareList, onDuplicateList }) => {
  ...
  const updateLists = (fn: (lists: ShoppingList[]) => ShoppingList[]) => onSaveLists(fn(lists));
  ...
  const toggleArchiveList = (id: string) => {
    updateLists(ls => ls.map(l => l.id === id ? { ...l, isArchived: !l.isArchived, updatedAt: new Date().toISOString() } : l));
  ...
  const saveListMeta = () => {
    ...
    updateLists(ls => ls.map(l => l.id === activeListId ? {
      ...l, name: editListName.trim(), budget: editListBudget ? parseFloat(editListBudget) : undefined,
      store: editListStore.trim() || undefined, color: editListColor, updatedAt: new Date().toISOString(),
    } : l));
  ...
  const addItem = () => {
    ...
    const newItem: ShopItem = {
      id: uid(), name: newItemName.trim(), qty: parseFloat(newItemQty) || 1, unit: newItemUnit,
      price: newItemPrice ? parseFloat(newItemPrice) : undefined, category: newItemCat,
      note: newItemNote.trim() || undefined, checked: false, addedAt: new Date().toISOString(),
    };
    updateLists(ls => ls.map(l => l.id === activeListId ? { ...l, items: [...l.items, newItem], updatedAt: new Date().toISOString() } : l));
    setNewItemName(''); setNewItemQty('1'); setNewItemPrice(''); setNewItemNote('');
  };

  const toggleItemCheck = (itemId: string) => {
    updateLists(ls => ls.map(l => l.id === activeListId ? {
      ...l, items: l.items.map(i => i.id === itemId ? { ...i, checked: !i.checked } : i), updatedAt: new Date().toISOString(),
    } : l));
  };
  ...
  const saveEditItem = (itemId: string) => {
    ...
    updateLists(ls => ls.map(l => l.id === activeListId ? {
      ...l, items: l.items.map(i => i.id === itemId ? {
        ...i, name: editItemName.trim(), qty: parseFloat(editItemQty) || 1, unit: editItemUnit,
        price: editItemPrice ? parseFloat(editItemPrice) : undefined, note: editItemNote.trim() || undefined,
  ...
  const clearChecked = () => {
    ...
    updateLists(ls => ls.map(l => l.id === activeListId ? {
      ...l, items: l.items.filter(i => !i.checked), updatedAt: new Date().toISOString(),
    } : l));
  ...
  const checkAll = () => {
    ...
    updateLists(ls => ls.map(l => l.id === activeListId ? {
      ...l, items: l.items.map(i => ({ ...i, checked: !allChecked })), updatedAt: new Date().toISOString(),
    } : l));
  };
    ...
    <div className="p-4 space-y-4 pb-20 animate-fadeIn">
      <div className="bg-slate-800/60 border border-slate-700/60 rounded-2xl p-3 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setIsCreatingList(true)} className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 px-3 py-1.5 rounded-xl font-bold text-xs flex items-center gap-1 transition-all active:scale-95">
            <Plus size={14} /><span>قائمة جديدة</span>
          </button>
        </div>
      ...
      <div className="flex gap-2">
        <div className="flex-1 relative">
          ...
          <input type="text" dir="auto" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="ابحث في القوائم..." className="w-full bg-slate-900/60 text-slate-100 text-xs rounded-xl pr-9 pl-3 py-2 border border-slate-800 focus:outline-none focus:border-emerald-500/50 placeholder:text-slate-600" />
        </div>
        <button onClick={() => setShowArchived(p => !p)} className={`px-3 py-2 rounded-xl border text-xs flex items-center gap-1 transition-all ${showArchived ? 'bg-amber-500/15 border-amber-500 text-amber-400' : 'bg-slate-900/60 border-slate-800 text-slate-400'}`}>
          <Archive size={14} /><span className="hidden sm:inline">الأرشيف</span>
        </button>
      </div>

      {isCreatingList && (
        <div className="bg-slate-900/80 border border-emerald-500/30 rounded-2xl p-4 space-y-3 animate-fadeIn">
          <h3 className="text-xs font-bold text-emerald-300 flex items-center gap-1.5"><ShoppingBag size={14} /> قائمة تسوق جديدة</h3>
          <input type="text" dir="auto" value={newListName} onChange={e => setNewListName(e.target.value)} placeholder="اسم القائمة (مثال: أسبوعية، رمضان...)" className="w-full bg-slate-950 text-slate-100 text-xs rounded-xl px-3 py-2 border border-slate-800 focus:outline-none focus:border-emerald-500/50" autoFocus onKeyDown={e => e.key === 'Enter' && createList()} />
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-slate-500 block mb-1">الميزانية (اختياري)</label>
              <input type="number" value={newListBudget} onChange={e => setNewListBudget(e.target.value)} placeholder="مثال: 500" className="w-full bg-slate-950 text-slate-100 text-xs rounded-xl px-3 py-2 border border-slate-800 focus:outline-none focus:border-emerald-500/50" />
            </div>
            <div>
              <label className="text-[10px] text-slate-500 block mb-1">اسم المتجر (اختياري)</label>
              <input type="text" dir="auto" value={newListStore} onChange={e => setNewListStore(e.target.value)} placeholder="مثال: كارفور" className="w-full bg-slate-950 text-slate-100 text-xs rounded-xl px-3 py-2 border border-slate-800 focus:outline-none focus:border-emerald-500/50" />
            </div>
          </div>
          <div className="flex items-center gap-2">
            {LIST_COLORS.map(c => (
              <button key={c.id} onClick={() => setNewListColor(c.id)} className={`w-7 h-7 rounded-full ${c.bg} flex items-center justify-center transition-all ${newListColor === c.id ? `ring-2 ring-offset-2 ring-offset-slate-900 ${c.ring} scale-110` : 'opacity-60 hover:opacity-90'}`}>
                {newListColor === c.id && <Check size={12} className="text-white stroke-[3]" />}
              </button>
            ))}
          </div>
          <div className="flex gap-2">
            <button onClick={createList} className="flex-1 bg-emerald-500 hover:bg-emerald-400 text-slate-950 py-2 rounded-xl text-xs font-bold">إنشاء القائمة</button>
            <button onClick={() => setIsCreatingList(false)} className="flex-1 bg-slate-800 text-slate-300 py-2 rounded-xl text-xs">إلغاء</button>
          </div>
        </div>
      )}
      ...
      {filteredLists.length === 0 ? (
        ...
        <div className="grid gap-3">
          {filteredLists.map(list => {
            ...
            return (
              <button key={list.id} onClick={() => { setActiveListId(list.id); setView('detail'); setFilterCat('all'); }} className={`text-right border rounded-2xl p-4 transition-all hover:scale-[1.01] active:scale-[0.99] w-full ${colorMeta.chip}`}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                    {onShareList && <button onClick={() => onShareList(list)} className="p-1 text-slate-400 hover:text-sky-400 rounded"><Share2 size={13} /></button>}
                    {onDuplicateList && <button onClick={() => onDuplicateList(list.id)} className="p-1 text-slate-400 hover:text-purple-400 rounded"><CopyPlus size={13} /></button>}
                    <button onClick={() => toggleArchiveList(list.id)} className="p-1 text-slate-400 hover:text-amber-400 rounded">{list.isArchived ? <RefreshCw size={13} /> : <Archive size={13} />}</button>
                    <button onClick={() => deleteList(list.id)} className="p-1 text-slate-400 hover:text-rose-400 rounded"><Trash2 size={13} /></button>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1.5 text-[10px] mb-3">
                  {list.store && <span className="flex items-center gap-0.5 bg-slate-900/40 px-1.5 py-0.5 rounded"><Store size={9} />{list.store}</span>}
                  {list.budget && <span className="flex items-center gap-0.5 bg-slate-900/40 px-1.5 py-0.5 rounded"><DollarSign size={9} />ميزانية {list.budget.toLocaleString('ar-EG')}</span>}
                </div>
                {total > 0 && (
                  <>
                    ...
                    <div className="flex items-center justify-between text-[10px]">
                      ...
                      {totalCost > 0 && <span className="text-slate-400">{cost.toFixed(1)} / {totalCost.toFixed(1)}</span>}
                    </div>
                  </>
                )}
      ...
      <div className="flex flex-col h-full animate-fadeIn">
        <div className={`${colorMeta.chip} border-b px-4 py-3 shrink-0`}>
          <div className="flex items-center justify-between gap-2">
            <button onClick={() => { setView('home'); setAddItemMode(false); setEditingListMeta(false); }} className="flex items-center gap-1 bg-slate-900/50 text-slate-300 hover:text-white py-1 pr-1 pl-2.5 rounded-lg text-xs">← رجوع</button>
            <div className="flex items-center gap-1">
              {onShareList && <button onClick={() => onShareList(activeList)} className="p-1.5 bg-slate-900/40 rounded-lg text-slate-300 hover:text-sky-400"><Share2 size={15} /></button>}
              {onDuplicateList && <button onClick={() => onDuplicateList(activeList.id)} className="p-1.5 bg-slate-900/40 rounded-lg text-slate-300 hover:text-purple-400"><CopyPlus size={15} /></button>}
              <button onClick={() => { setEditListName(activeList.name); setEditListBudget(activeList.budget?.toString() ?? ''); setEditListStore(activeList.store ?? ''); setEditListColor(activeList.color); setEditingListMeta(p => !p); }} className="p-1.5 bg-slate-900/40 rounded-lg text-slate-300 hover:text-amber-400"><Edit3 size={15} /></button>
              <button onClick={checkAll} className="p-1.5 bg-slate-900/40 rounded-lg text-slate-300 hover:text-emerald-400"><CheckCircle2 size={15} /></button>
              <button onClick={() => setAddItemMode(p => !p)} className={`px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1 transition-all ${addItemMode ? 'bg-slate-700 text-slate-300' : 'bg-emerald-500 text-slate-950'}`}>
                {addItemMode ? <X size={14} /> : <Plus size={14} />}<span>{addItemMode ? 'إلغاء' : 'إضافة'}</span>
              </button>
            </div>
          </div>
          <h2 className="text-base font-extrabold text-white mt-2">{activeList.name}</h2>
          <div className="mt-2 space-y-1">
            <div className="h-2 bg-slate-900/40 rounded-full overflow-hidden"><div className={`h-full ${colorMeta.bg} rounded-full transition-all duration-500`} style={{ width: `${pct}%` }} /></div>
            <div className="flex items-center justify-between text-[10px]">
              <span className="text-slate-300">{done}/{total} منتج · {pct}%</span>
              {totalCost > 0 && <span className={overBudget ? 'text-rose-400 font-bold' : 'text-slate-300'}>{cost.toFixed(1)} / {activeList.budget ? `${activeList.budget} (ميزانية)` : totalCost.toFixed(1)}{overBudget && ' ⚠️'}</span>}
            </div>
          </div>
        ...
        {editingListMeta && (
          <div className="bg-slate-900 border-b border-slate-800 p-3 space-y-2 shrink-0 animate-fadeIn">
            <input type="text" dir="auto" value={editListName} onChange={e => setEditListName(e.target.value)} placeholder="اسم القائمة" className="w-full bg-slate-950 text-slate-100 text-xs rounded-xl px-3 py-2 border border-slate-800 focus:outline-none focus:border-amber-500/50" />
            <div className="grid grid-cols-2 gap-2">
              ...
              <input type="text" dir="auto" value={editListStore} onChange={e => setEditListStore(e.target.value)} placeholder="المتجر" className="bg-slate-950 text-slate-100 text-xs rounded-xl px-3 py-2 border border-slate-800 focus:outline-none w-full" />
            </div>
        ...
        {addItemMode && (
          <div className="bg-slate-900 border-b border-slate-800 p-3 space-y-2.5 shrink-0 animate-fadeIn">
            <div className="flex gap-1.5">
              <input type="text" dir="auto" value={newItemName} onChange={e => setNewItemName(e.target.value)} onKeyDown={e => e.key === 'Enter' && addItem()} placeholder="اسم المنتج..." className="flex-1 bg-slate-950 text-slate-100 text-xs rounded-xl px-3 py-2 border border-slate-800 focus:outline-none focus:border-emerald-500/50 placeholder:text-slate-600" autoFocus />
              <button onClick={addItem} className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 px-3 rounded-xl font-bold text-lg transition-all active:scale-90"><Plus size={18} /></button>
            </div>
            <div className="grid grid-cols-3 gap-1.5">
              <div>
                <label className="text-[9px] text-slate-500 block mb-0.5">الكمية</label>
                <input type="number" min="0.1" step="0.1" value={newItemQty} onChange={e => setNewItemQty(e.target.value)} className="w-full bg-slate-950 text-slate-100 text-[11px] rounded-lg px-2 py-1.5 border border-slate-800 focus:outline-none" />
              </div>
              <div>
                <label className="text-[9px] text-slate-500 block mb-0.5">الوحدة</label>
                <select value={newItemUnit} onChange={e => setNewItemUnit(e.target.value as ShopItemUnit)} className="w-full bg-slate-950 text-slate-300 text-[11px] rounded-lg px-1.5 py-1.5 border border-slate-800 focus:outline-none">
              ...
              <div>
                ...
                <input type="number" min="0" step="0.1" value={newItemPrice} onChange={e => setNewItemPrice(e.target.value)} placeholder="0.00" className="w-full bg-slate-950 text-slate-100 text-[11px] rounded-lg px-2 py-1.5 border border-slate-800 focus:outline-none" />
              </div>
            </div>
            <div className="flex gap-1 overflow-x-auto no-scrollbar pb-0.5">
              {SHOP_CATEGORIES.map(cat => (
                <button key={cat.id} onClick={() => setNewItemCat(cat.id)} className={`text-[10px] px-2 py-1 rounded-lg border whitespace-nowrap flex items-center gap-0.5 transition-all ${newItemCat === cat.id ? 'bg-emerald-500/15 border-emerald-500 text-emerald-300 font-bold' : 'bg-slate-900 border-slate-800 text-slate-500'}`}>
                  <span>{cat.emoji}</span><span>{cat.label}</span>
                </button>
              ))}
            </div>
            <input type="text" dir="auto" value={newItemNote} onChange={e => setNewItemNote(e.target.value)} placeholder="ملاحظة (اختياري): ماركة، لون، مقاس..." className="w-full bg-slate-950 text-slate-100 text-[11px] rounded-xl px-3 py-2 border border-slate-800 focus:outline-none focus:border-emerald-500/50 placeholder:text-slate-600" />
          </div>
        )}
        ...
        {categoriesInList.length > 1 && (
          <div className="px-3 py-2 border-b border-slate-800 bg-slate-950/40 shrink-0">
            <div className="flex gap-1 overflow-x-auto no-scrollbar">
              <button onClick={() => setFilterCat('all')} className={`text-[10px] px-2 py-1 rounded-lg border whitespace-nowrap transition-all ${filterCat === 'all' ? 'bg-amber-500 text-slate-950 border-amber-400 font-bold' : 'bg-slate-900 border-slate-800 text-slate-500'}`}>الكل ({activeList.items.length})</button>
              ...
              return (
                <div key={catKey} className="space-y-1.5">
                  <div className="flex items-center gap-1.5 text-[11px] text-slate-400 font-bold px-1">
                    <span>{catMeta.emoji}</span><span>{catMeta.label}</span><span className="text-[10px] font-normal text-slate-600">({doneInCat}/{items.length})</span>
                  ...
                  {items.map(item => {
                    ...
                    return (
                      <div key={item.id} className={`bg-slate-900/50 border rounded-2xl px-3 py-2.5 transition-all ${item.checked ? 'border-slate-800/40 opacity-60' : 'border-slate-800'}`}>
                        {isEditing ? (
                          <div className="space-y-2">
                            <input type="text" dir="auto" value={editItemName} onChange={e => setEditItemName(e.target.value)} className="w-full bg-slate-950 text-slate-100 text-xs rounded-xl px-3 py-2 border border-slate-800 focus:outline-none focus:border-amber-500/50" />
                            <div className="grid grid-cols-3 gap-1.5">
                            ...
                            <input type="text" dir="auto" value={editItemNote} onChange={e => setEditItemNote(e.target.value)} placeholder="ملاحظة" className="w-full bg-slate-950 text-slate-100 text-[11px] rounded-xl px-3 py-2 border border-slate-800 focus:outline-none" />
                            <div className="flex gap-1.5">
                          ...
                          <>
                            <div className="flex items-center gap-2.5">
                              ...
                              <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpandedItemId(expandedItemId === item.id ? null : item.id)}>
                                ...
                                {item.note && !expandedItemId?.includes(item.id) && <p className="text-[10px] text-slate-500 truncate mt-0.5">{item.note}</p>}
                              </div>
                              <div className="flex items-center gap-0.5 shrink-0">
                                <button onClick={() => { setEditingItemId(item.id); setEditItemName(item.name); setEditItemQty(item.qty.toString()); setEditItemUnit(item.unit); setEditItemPrice(item.price?.toString() ?? ''); setEditItemNote(item.note ?? ''); }} className="p-1 text-slate-600 hover:text-amber-400 rounded"><Edit3 size={12} /></button>
                                <button onClick={() => deleteItem(item.id)} className="p-1 text-slate-600 hover:text-rose-400 rounded"><Trash2 size={12} /></button>
                              </div>
                            </div>
                            {expandedItemId === item.id && item.note && <p className="text-[11px] text-slate-400 mt-1.5 pr-7 leading-relaxed">{item.note}</p>}
                          </>
                        )}
                      </div>
                    );
        ...
        {activeList.items.some(i => i.checked) && (
          <div className="border-t border-slate-800 bg-slate-900 p-3 flex items-center justify-between shrink-0">
            ...
            <button onClick={clearChecked} className="text-xs bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 border border-rose-500/20 px-3 py-1.5 rounded-xl flex items-center gap-1 transition-colors">
              <Trash2 size={13} /><span>حذف المشتراة</span>
            </button>
          </div>
        )}
      </div>
